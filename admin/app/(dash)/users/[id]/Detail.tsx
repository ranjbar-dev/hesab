...
const input =
  "h-11 w-full rounded-lg border border-brand-border bg-brand-bg px-3 focus-visible:ring-2 focus-visible:ring-brand-accent/30";
const button =
  "h-11 cursor-pointer rounded-lg border border-brand-border px-4 transition-colors duration-200 hover:bg-brand-bg focus-visible:ring-2 focus-visible:ring-brand-accent/30";

function Info({ l, children }: { l: string; children: React.ReactNode }) {
  return (
    <div>
      <dt className="text-brand-muted">{l}</dt>
      <dd>{children}</dd>
    </div>
  );
}
...
export default function Detail() {
      ...
      setU(
        (
          await updateUser(u.id, {
            ...
            national_id: String(f.get("national_id")),
  ...
  const addBusiness = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await createBusiness({ name: businessName, owner_user_id: u.id });
      setBusinessName("");
  ...
  const removeFromBusiness = async (businessID: number) => {
    if (!confirm("این کاربر از کسب‌وکار حذف شود؟")) return;
    try {
      await removeBusinessMember(businessID, u.id);
  ...
  return (
    <main className="mx-auto max-w-2xl p-6 sm:p-12">
      <Link
        href="/users"
        className="rounded-lg text-sm text-brand-muted focus-visible:ring-2 focus-visible:ring-brand-accent/30"
      >
        → بازگشت به فهرست
      </Link>
      <section className="mt-6 rounded-2xl border border-brand-border bg-brand-surface p-6">
        ...
        <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
          <Info l="موبایل">
            ...
            <small className="block text-brand-muted">
      ...
      <form
        onSubmit={edit}
        className="mt-5 space-y-3 rounded-2xl border border-brand-border bg-brand-surface p-6"
      >
        ...
        <Select
          ...
          value={{
            ...
            label: u.account_type === "company" ? "حقوقی" : "حقیقی",
          }}
          onChange={(v) =>
            setU((current) =>
              current
                ? {
                    ...current,
                    account_type: (v?.value ??
                      current.account_type) as User["account_type"],
                  }
                : current,
            )
          }
          ...
          aria-label="نوع حساب"
        />
        <button className="h-11 cursor-pointer rounded-lg bg-brand-accent px-5 font-semibold text-brand-bg transition-colors duration-200 hover:bg-brand-accent-hover focus-visible:ring-2 focus-visible:ring-brand-accent/30">
          ذخیره تغییرات
        </button>
      </form>
      ...
      <section className="mt-5 rounded-2xl border border-brand-border bg-brand-surface p-6">
        ...
        <div className="mt-4 space-y-5">
          <button onClick={toggle} className={button}>
            {u.status === "active" ? "غیرفعال‌سازی حساب" : "فعال‌سازی حساب"}
          </button>
          ...
          <button
            onClick={remove}
            className="cursor-pointer text-red-400 focus-visible:ring-2 focus-visible:ring-brand-accent/30"
          >
            حذف کاربر
          </button>
        </div>
      ...
      <section className="mt-5 rounded-2xl border border-brand-border bg-brand-surface p-6">
        ...
        <h3 className="mt-5 text-sm font-semibold text-brand-muted">
          کسب‌وکارهای تحت مالکیت
        </h3>
            ...
            owned.map((b) => (
              ...
              >
                ...
                <span className="text-brand-muted">
                  {b.member_count.toLocaleString("fa-IR")} عضو ·{" "}
                  {isoToJalaliLabel(b.created_at)}
                </span>
              </div>
            ))
          ) : (
            <p className="text-sm text-brand-muted">کسب‌وکاری ندارد</p>
          )}
        </div>
        <form onSubmit={addBusiness} className="mt-3 flex gap-2">
          <input
            required
            value={businessName}
            onChange={(e) => setBusinessName(e.target.value)}
            placeholder="نام کسب‌وکار جدید"
            className={`${input} flex-1`}
            aria-label="نام کسب‌وکار جدید"
          />
          <button className={button}>افزودن کسب‌وکار</button>
        </form>
        <h3 className="mt-6 text-sm font-semibold text-brand-muted">
          عضویت‌ها
        </h3>
        <div className="mt-2 space-y-2">
          {joined.length ? (
            joined.map((b) => (
              <div
                key={b.id}
                className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-brand-border p-3 text-sm"
              >
                <span>
                  <Link
                    href={`/businesses/${b.id}`}
                  ...
                  <small className="mr-2 text-brand-muted">
                    {roleLabels[b.role]} · مالک: {b.owner_name}
                  </small>
                </span>
                <button
            ...
            <p className="text-sm text-brand-muted">عضویتی ندارد</p>
          )}
        </div>
      </section>
    </main>
